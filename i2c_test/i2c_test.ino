...
int i2cWrite(uint8_t reg, uint8_t val) {  // write reg via i2c
    xPrintf("i2cWrite START reg %02x val %02x" EOL, reg, val);
    ...
    xPrintf("i2cWrite doing i2c_write_blocking() reg %02x val %02x" EOL, reg, val);
    ...
    xPrint(EOL);
    if (res==PICO_ERROR_GENERIC)  xPrintf("ERROR: i2cWrite() got bad res %d reg %02x val %02x" EOL, res, reg, val);
    else if (res==2) xPrintf("GOOD: i2cWrite() got good res %d reg %02x val %02x" EOL, res, reg, val);
    else xPrintf("UNEXPECTED: i2cWRite() got unexpected res %d reg %02x val %02x" EOL, res, reg, val);
    xPrint(EOL);

    xPrintf("i2cWrite END reg %02x val %02x" EOL, reg, val);
    return res;
}
...
int i2cRead(uint8_t reg, uint8_t val) {  // read reg via i2c
    xPrintf("i2cRead START reg %02x val %02x" EOL, reg, val);
    ...
    xPrint(F("Doing i2c_read_blocking()" EOL));
    ...
    if (res==PICO_ERROR_GENERIC)  
        xPrintf("ERROR: i2cRead() got bad res %d reg %02x val %02x" EOL, res, reg, val);
    else if (res==2) 
        xPrintf("GOOD: i2cRead() got good res %d reg %02x val %02x" EOL, res, reg, val);
    else 
        xPrintf("UNEXPECTED: i2cRead() got unexpected res %d reg %02x val %02x" EOL, res, reg, val);
    xPrint(EOL);


    xPrintf("i2cRead END reg %02x val %02x" EOL, reg, val);
    return res;
}
//****************************************
void loop() {
    ...
    while (true) {
        i++;
        xPrintf("while loop %d start" EOL, i);
        if (i > 25) {
            xPrint(EOL);
            xPrint("start over at 0" EOL);
            xPrint(EOL);
            reg_addr = 0;
            break;
        }
        ...
        bool bad = false;
        xPrint(EOL);
        xPrint("i2cWrite() start" EOL);
        // result = i2cWrite(SI5351A_OUTPUT_ENABLE_CONTROL, 0xff) ;
        result = i2cWrite(reg_addr, 0x55) ;
        xPrint("i2cWrite() complete" EOL);
        if (result == 2) {
            xPrintf("GOOD: result %d after i2cWrite" EOL, result);
            busy_wait_ms(3000);
        } else if (result == PICO_ERROR_GENERIC) {
            xPrintf("ERROR: result %d after i2cWrite" EOL, result);
            bad = true;
        } else {
            xPrintf("UNEXPECTED: result %d after i2cWrite" EOL, result);
            bad = true;
        }

        if (!bad) {
            xPrint(EOL);
            xPrint("i2cRead() start" EOL);
            // result = i2cRead(SI5351A_OUTPUT_ENABLE_CONTROL, 0xff) ;
            result = i2cRead(reg_addr, 0xaa) ;
            xPrint("i2cRead() complete" EOL);

            if (result == 2) {
                xPrintf("GOOD: result %d after i2cRead" EOL, result);
                busy_wait_ms(3000);
            } else if (result == PICO_ERROR_GENERIC) {
                xPrintf("ERROR: result %d after i2cRead" EOL, result);
                bad = true;
            } else {
                xPrintf("UNEXPECTED: result %d after i2cRead" EOL, result);
                bad = true;
            }
        }
        ...
        if (!bad) {
            xPrint(EOL);
            reg_addr = reg_addr + 1;
            xPrintf("incremented reg_addr %u" EOL, reg_addr);
            xPrint(EOL);
            break;
        }
        ...
        //******************************************
        xPrint(EOL);
        xPrintln("i2c_deinit() start" EOL);
        i2c_deinit(VFO_I2C_INSTANCE);
        xPrintln("i2c_deinit() complete" EOL);
        busy_wait_ms(1000);

        // https://cec-code-lab.aps.edu/robotics/resources/pico-c-api/group__hardware__i2c.html
        xPrintln("i2c_init() start" EOL);
        i2c_init(VFO_I2C_INSTANCE, VFO_I2C0_SCL_HZ);
        xPrintln("i2c_init() complete" EOL);

        gpio_init(VFO_I2C0_SCL_PIN);
        gpio_init(VFO_I2C0_SDA_PIN);

        xPrintln("no pullup or pulldowns by RP2040 on both SDA and SCL.. external R on pcb");
        gpio_set_pulls(VFO_I2C0_SDA_PIN, false, false);
        gpio_set_pulls(VFO_I2C0_SCL_PIN, false, false);
        ...
        xPrintln("i2c_set_function() start" EOL);
        gpio_set_function(VFO_I2C0_SDA_PIN, GPIO_FUNC_I2C);
        gpio_set_function(VFO_I2C0_SCL_PIN, GPIO_FUNC_I2C);
        i2c_set_slave_mode(VFO_I2C_INSTANCE, false, 0);
        xPrintln("i2c_set_function() complete" EOL);
        digitalWrite(Si5351Pwr, HIGH);
        ...
        xPrint(EOL);
        xPrintf("while loop end %d" EOL, i);
